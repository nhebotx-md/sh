...
async function getJadwalSholat(kota = "Jakarta") {
    try {
        // Panggil API Aladhan berdasarkan kota
        const response = await axios.get(`https://api.aladhan.com/v1/timingsByCity?city=${encodeURIComponent(kota)}&country=Indonesia&method=2`);
        const data = response.data.data.timings;

        // Konversi waktu ke format WIB (Asia/Jakarta)
        function convertToWIB(time) {
            return moment(time, "HH:mm").tz("Asia/Jakarta").format("HH:mm");
        }

        return {
            tanggal: moment().tz("Asia/Jakarta").format("dddd, D MMMM YYYY"),
            lokasi: kota,
            subuh: convertToWIB(data.Fajr),
            dzuhur: convertToWIB(data.Dhuhr),
            ashar: convertToWIB(data.Asr),
            maghrib: convertToWIB(data.Maghrib),
            isya: convertToWIB(data.Isha),
        };
    } catch (error) {
        console.error("❌ Gagal mengambil jadwal sholat:", error.message);
        return { error: "⚠️ Gagal mengambil jadwal sholat. Silakan coba lagi nanti." };
    }
}
...
module.exports = { pinterest, pinterest2, wallpaper, remini, wikimedia, quotesAnime, multiDownload, yanzGpt, happymod, mediafireDl, umma, ringtone, getJadwalSholat, styletext, tiktokDl, facebookDl, instaStory, bk9Ai, ytMp4, ytMp3, quotedLyo, simi }
